...
module.exports = {
    ...
    getByEmail: (email) => {
        return new Promise((resolve, reject) => {
            connection.query('SELECT id_user, fullname, id_card, b.role, email, fullname,salt, password, created_at, updated_at FROM tb_users a JOIN tb_role b ON a.id_role = b.id_role WHERE email = ?', email, (err, result) => {
                if (!err) {
                    resolve(result)
                } else {
                    reject(new Error(err))
